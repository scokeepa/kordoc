...
> ```powershell
> # 방법 1 — .cmd 확장자로 바꿔 실행 (가장 간단, 시스템 설정 불변)
> npx.cmd -y kordoc setup
>
> # 방법 2 — PowerShell 실행 정책 완화 (한 번만)
> Set-ExecutionPolicy -Scope CurrentUser RemoteSigned
> npx -y kordoc setup
> ```
> 또는 PowerShell 대신 **명령 프롬프트(cmd)** 창에서 `npx -y kordoc setup` 그대로 실행해도 됩니다.
